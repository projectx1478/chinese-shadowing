...
2. 「ステップガイド」を選ぶと、①の内容をもとにGemini APIが2〜4個の意味のまとまり（例：時間→誰が→何をする→対象）に分解し、各ステップに日本語の短いヒント（使えそうな単語・文法）を添えて返す
3. ユーザーは1ステップずつ中国語を入力し「次へ」で進む（「戻る」で前のステップの入力を保持したまま戻れる）。**ステップごとのAI添削は行わない**（後述の理由により追加API呼び出しを避けるため）
4. 最終ステップ完了で各ステップの回答を結合（スペースなしで連結、中国語の表記慣習に合わせる）し、既存の②テキストエリア（`#compose-zh-input`）にそのまま流し込む。以降は既存の「③添削してもらう」ボタン・`submitCompose()`にそのままつながる（**無修正**）
...
**実装状況**：実装済み
- 状態は`composeGuide`というモジュールスコープのオブジェクト（`{mode, status, steps, index, current, answers}`）で管理。`S`グローバル状態には乗せていない（作文練習全体が`S`と独立した自己完結実装のため、8-6の設計判断を踏襲）
...
- `finishComposeGuide()`が最終的に`#compose-zh-input`へ値を設定するだけなので、`submitCompose()`側は一切変更していない
