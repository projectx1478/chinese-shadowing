...
### 8-6d. ステップガイドのヒント表示改善（段階的表示）

**課題**：ステップガイドのヒントに「拼音付きの具体的な中国語表現例」がそのまま表示されていたため、学習者がヒントを見れば実質的に答えが分かってしまい、自分で考える過程を失わせていた。

**対応**
- Gemini プロンプト修正：ヒント情報を「文字数・品詞・先頭文字・語義の方向性」など最小限の情報のみに指定。具体的な中国語表現例は返さない
- UI 改善：各ステップで最初はヒント非表示 → 学習者が「ヒントを見る」ボタンをクリックして段階的に表示 → 次ステップに進むと新しいステップのヒントは再び非表示にリセット（毎回自分で考える環境を提供）
- 状態管理：`composeGuide.hintsShown` 配列でステップごとのヒント表示状態を管理
- 上記はプロンプト・UI 改善のみで、API 呼び出し回数（計2回で固定）に変更なし

**実装状況**：実装済み
- Playwright テストを1件追加（ヒント非表示 → ボタンクリックで表示 → 次ステップでリセットの確認）
